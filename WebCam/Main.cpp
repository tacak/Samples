...
int main()
{
	...
	for(int i = 0; i < iCount; i++){
		pConfig->GetStreamCaps(i, &amt, (BYTE *)&vsc);
		sprintf(buf, "X=%d , Y=%d\r\nに設定しますか", vsc.MinCroppingSize.cx, vsc.MaxCroppingSize.cy);
		ret = MessageBox(NULL, buf, "Message", MB_YESNO);
		if(ret == IDYES){
	...
	NULL, pDeviceFilter, pSampleGrabberFilter, pVideoRenderer);
